...
class RNNModel(nn.Module):
    ...
    def predict(self, inputs, targets, ntokens, usecuda = True):
        ...
        with torch.no_grad():
            ...
            outputs = outputs.view(max_len*batch_size, ntokens)
            scores = self.criterion(outputs, targets.view(-1))
        
        scores = scores.view(max_len, batch_size)
        scores = scores.t().sum(dim=1)
