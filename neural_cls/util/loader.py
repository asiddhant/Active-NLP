...
class Loader(object):
    ...
    def load_subj(self, datapath, pretrained, word_dim = 100):
        
        trainpath = os.path.join(datapath, 'subj.train')
        validpath = os.path.join(datapath, 'subj.valid')
        testpath = os.path.join(datapath, 'subj.test')
        
        train_data = []
        with open (trainpath) as f:
            for line in f:
                content = line.strip().split('\t')
                tag, sentence = content
                train_data.append((sentence, tag))  
        ...
        with open (validpath) as f:
            for line in f:
                content = line.strip().split('\t')
                tag, sentence = content
                valid_data.append((sentence, tag))       
        ...
        dico_words, word_to_id, id_to_word = augment_with_pretrained(
                ...
                pretrained,
                list(itertools.chain.from_iterable(
                    [[w.lower() for w in s[0].split()] for s in valid_data + test_data])
                ) if not all_embedding else None)
        
        dico_tags, tag_to_id, id_to_tag = tag_mapping(train_data)
        
        train_data_final = prepare_dataset(train_data, word_to_id, tag_to_id)
        valid_data_final = prepare_dataset(valid_data, word_to_id, tag_to_id)
        test_data_final = prepare_dataset(test_data, word_to_id, tag_to_id)
        ...
        return train_data_final, valid_data_final, test_data_final, mappings
